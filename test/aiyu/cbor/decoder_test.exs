defmodule Aiyu.CBOR.DecoderTest do
  @moduledoc """
  ...
  describe "basic data model" do
    test "decode/1" do
      table = [
        ...
        {%CBOR.Tag{number: 2, content: <<0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>},
         <<0xC2, 0x49, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>},
        {-18_446_744_073_709_551_616, <<0x3B, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF>>},
        {%CBOR.Tag{number: 3, content: <<0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>},
        ...
        {:NaN, <<0xF9, 0x7E, 0x00>>},
        {:"-inf", <<0xF9, 0xFC, 0x00>>},
        {:inf, <<0xFA, 0x7F, 0x80, 0x00, 0x00>>},
        {:NaN, <<0xFA, 0x7F, 0xC0, 0x00, 0x00>>},
        {:"-inf", <<0xFA, 0xFF, 0x80, 0x00, 0x00>>},
        {:inf, <<0xFB, 0x7F, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>},
        {:NaN, <<0xFB, 0x7F, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>},
        {:"-inf", <<0xFB, 0xFF, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>},
        {%CBOR.SimpleValue{value: 20}, <<0xF4>>},
        {%CBOR.SimpleValue{value: 21}, <<0xF5>>},
        {%CBOR.SimpleValue{value: 22}, <<0xF6>>},
        ...
        {%CBOR.Tag{number: 23, content: <<0x01, 0x02, 0x03, 0x04>>},
         <<0xD7, 0x44, 0x01, 0x02, 0x03, 0x04>>},
        {%CBOR.Tag{number: 24, content: <<0x64, 0x49, 0x45, 0x54, 0x46>>},
        ...
        {[], <<0x9F, 0xFF>>},
        {[1, [2, 3], [4, 5]], <<0x9F, 0x01, 0x82, 0x02, 0x03, 0x9F, 0x04, 0x05, 0xFF, 0xFF>>},
        {[1, [2, 3], [4, 5]], <<0x9F, 0x01, 0x82, 0x02, 0x03, 0x82, 0x04, 0x05, 0xFF>>},
        {[1, [2, 3], [4, 5]], <<0x83, 0x01, 0x82, 0x02, 0x03, 0x9F, 0x04, 0x05, 0xFF>>},
        {[1, [2, 3], [4, 5]], <<0x83, 0x01, 0x9F, 0x02, 0x03, 0xFF, 0x82, 0x04, 0x05>>},
        {[
           1,
           2,
           3,
        ...
        {["a", %{"b" => "c"}], <<0x82, 0x61, 0x61, 0xBF, 0x61, 0x62, 0x61, 0x63, 0xFF>>},
        {%{"Fun" => %CBOR.SimpleValue{value: 21}, "Amt" => -2},
         <<0xBF, 0x63, 0x46, 0x75, 0x6E, 0xF5, 0x63, 0x41, 0x6D, 0x74, 0x21, 0xFF>>}
      ]

      decoder = CBOR.Decoder.basic()

      for {expected, encoded} <- table do
        assert {:ok, ^expected, <<>>} = CBOR.Decoder.decode(decoder, encoded)
      end
    end
  end

  describe "extended data model" do
    test "decode/1" do
      table = [
        {0, <<0x00>>},
        {1, <<0x01>>},
        ...
        {%CBOR.SimpleValue{value: 16}, <<0xF0>>},
        {%CBOR.SimpleValue{value: 255}, <<0xF8, 0xFF>>},
        {DateTime.new!(~D[2013-03-21], ~T[20:04:00]),
         <<0xC0, 0x74, 0x32, 0x30, 0x31, 0x33, 0x2D, 0x30, 0x33, 0x2D, 0x32, 0x31, 0x54, 0x32,
           0x30, 0x3A, 0x30, 0x34, 0x3A, 0x30, 0x30, 0x5A>>},
        {~U[2013-03-21 20:04:00.000000Z], <<0xC1, 0x1A, 0x51, 0x4B, 0x67, 0xB0>>},
        {~U[2013-03-21 20:04:00.500000Z],
         <<0xC1, 0xFB, 0x41, 0xD4, 0x52, 0xD9, 0xEC, 0x20, 0x00, 0x00>>},
        {%CBOR.Tag{number: 23, content: <<0x01, 0x02, 0x03, 0x04>>},
         <<0xD7, 0x44, 0x01, 0x02, 0x03, 0x04>>},
        {%CBOR.Tag{number: 24, content: <<0x64, 0x49, 0x45, 0x54, 0x46>>},
